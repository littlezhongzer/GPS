...
public class GPS_cold extends AppCompatActivity {
    private LocationManager locationManager;
    private String locationProvider;
    public final  String TAG="gps cold";
    ...
    String mkName1="GPS/";
    String mkName2="Cold/";
    String private_filename="Cold_test.txt";  //test statistics


    String fileName_nema;
    String private_filename_nema="Cold_nema.txt"; //nema statistics
    ...
    protected void onCreate(Bundle savedInstanceState)  {
        ...
        setContentView(R.layout.activity_gps_cold);
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            if (ActivityCompat.checkSelfPermission(GPS_cold.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(GPS_cold.this, PERMISSIONS_STORAGE, REQUEST_PERMISSION_CODE);
     ...
     //   }
        ...
        GPS_cold_Thread cold_thread = new GPS_cold_Thread();
        cold_thread.start();
    ...
    private void showLocation(Location location) {
        if(!flag_send_msg_listener){
            Log.d(TAG,"定位成功------->"+"location------>维度为：" + location.getLatitude() + "\n经度为" + location.getLongitude());
        ...
        public void run(){
            ...
            while (true){
                if(flag_cold){
                    ...
                    //bundle=null;
                    //bundle.putBoolean("glo almanac",true);
                    //bundle.putBoolean("sbas almanac",true);
                    //locationManager.sendExtraCommand("gps", "delete_aiding_data", bundle); //冷启动
                    locationManager = (LocationManager) getSystemService(getApplicationContext().LOCATION_SERVICE);
                    locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle);
                    Log.d(TAG, "run: "+locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle));
                    Log.d(TAG, "run: "+locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle));
                    Log.d(TAG, "run: "+locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle));
                    Log.d(TAG, "run: "+locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle));
                    Log.d(TAG, "run: "+locationManager.sendExtraCommand(LocationManager.GPS_PROVIDER,"delete_aiding_data", bundle));
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 0x1001) {
                ...
                mFileutil.result_append(SD_path+mkName1+mkName2+fileName,mFileutil.getDate()+"\t"+"*GPS Cold Boot Up*"+"\t"+times_st+"\t"+diff_time_st+" ms"+"\t"+Longitude+"#"+Latitude+"\t"+count+"\t"+"PASS"+"\t\n");
            ...
            if(msg.what==0x1004){
                ...
                mFileutil.result_append(SD_path+mkName1+mkName2+fileName,mFileutil.getDate()+"\t"+"*GPS Cold Boot Up*"+"\t"+times_st+"\t"+"180000"+"\t"+"fail"+"#"+"fail"+"\t"+count+"\t"+"Time Out:Exceeded 180s"+"\t\n");
                flag_addlistener=false;
            ...
            if(msg.what==0x1005){
                if(i==0){
                    ...
                    flag_addlistener=false;
                    Toast.makeText(GPS_cold.this,"数据统计成功，请按返回键退出",Toast.LENGTH_SHORT).show();
                }
            }
        }
    };
    ...
    public void exit() {
        if ((System.currentTimeMillis() - mExitTime) > 2000) {
            View view;
            AlertDialog.Builder builder= new AlertDialog.Builder(GPS_cold.this);
            view= LayoutInflater.from(GPS_cold.this).inflate(R.layout.dialog, null);
    ...
    protected void onDestroy(){
        ...
        Location location=null;
        flag_cold=false;
        //Log.d(TAG, "onDestroy: hehehehehe");
    }
